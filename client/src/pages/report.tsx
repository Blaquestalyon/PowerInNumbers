...
import type { GenerationRun, GenerationSection, Stage2Question } from "@shared/schema";
...
export default function ReportPage() {
  const [, routeParams] = useRoute("/report/:runId");
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<RunWithSections>({
    queryKey: ["/api/runs", runId],
    enabled: !!runId,
    refetchInterval: (query) => {
      const data = query.state.data as RunWithSections | undefined;
  ...
  const emailMutation = useMutation({
    mutationFn: async () => {
      await apiRequest("POST", `/api/runs/${runId}/email`);
    },
    onSuccess: () => {
  ...
  const phase = useMemo(() => {
    ...
    if (!run.coreAnalysisA1a) return "core_analysis";
    const hasRunning = sections.some((s) => s.status === "running");
    const hasQueued = sections.some(
      (s) => s.status === "queued" || s.status === "ready"
    );
    if (hasRunning || hasQueued) return "sections";
    return "finishing";
  }, [run, sections]);
  ...
  useEffect(() => {
    const handler = () => {
      const sectionEls = document.querySelectorAll("[data-section-code]");
      let current: string | null = null;
      sectionEls.forEach((el) => {
        const rect = el.getBoundingClientRect();
        if (rect.top <= 120) {
          current = el.getAttribute("data-section-code");
        }
      });
  ...
  // Execute <script> tags inside dangerouslySetInnerHTML sections
  // React does NOT execute scripts injected via innerHTML — we must do it manually
  const executeScripts = useCallback(() => {
    if (!sectionsContainerRef.current) return;
    const scriptTags = sectionsContainerRef.current.querySelectorAll(
      ".report-section script"
    );
    scriptTags.forEach((script) => {
      const scriptContent = script.textContent || "";
      // Use a hash of first 100 chars + length as a dedup key
      const key = `${scriptContent.substring(0, 100)}__${scriptContent.length}`;
  ...
  // Run script execution whenever sections update
  useEffect(() => {
    const completedWithHtml = sections.filter(
      (s) => s.status === "complete" && s.htmlFragment
    );
    if (completedWithHtml.length > 0) {
      // Small delay to ensure DOM is updated
      const timer = setTimeout(executeScripts, 100);
      return () => clearTimeout(timer);
    }
  }, [sections, executeScripts]);
        ...
        <Card className="max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Could not fetch run data. Please try again.
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Badge
              ...
              className="ml-2"
            >
              {run.stage === 1 && run.status === "complete" ? "Stage 1 Complete" : run.stage === 3 && run.status === "complete" ? "Report Complete" : phaseLabel}
            </Badge>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                window.location.href = `/api/runs/${run.id}/export-html`;
              }}
              title="Download standalone interactive HTML (Monte Carlo works in the downloaded file)"
            >
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 flex gap-8">
              ...
              {sections.map((s) => (
                <a
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    document
                      .querySelector(`[data-section-code="${s.code}"]`)
                ...
                >
                  ...
                  <span className="truncate">{s.title ? `${s.code} ${s.title}` : s.code}</span>
        ...
        <main className="flex-1 min-w-0">
            ...
            <CardContent className="pt-6">
              ...
              <div className="flex gap-6 text-xs text-muted-foreground">
                ...
                <PhaseIndicator
                  ...
                  done={phase === "finishing" || phase === "complete"}
                />
                <PhaseIndicator
                  label="Finishing"
          ...
          {/* Report Sections */}
          <div className="space-y-6" ref={sectionsContainerRef}>
            {sections.map((section) => (
              ...
              >
                {section.status === "complete" && section.htmlFragment ? (
                  <div
                    className="report-section animate-in fade-in duration-500"
                    dangerouslySetInnerHTML={{
                      __html: section.htmlFragment,
                    }}
                  />
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => retryMutation.mutate(section.id)}
          ...
          {/* Completion state — stage-aware */}
          {phase === "complete" && run.stage === 1 && (
            <Stage2Enrichment runId={parseInt(runId!, 10)} />
          )}
          {phase === "complete" && (run.stage === 3 || run.stage === 2) && (
            <div className="mt-12 text-center pb-12">
              ...
              <p className="text-muted-foreground text-sm">
                {completedSections.length} sections generated successfully.
              </p>
            </div>
          )}
...
// ═══════════════════════════════════════════════════════════
// STAGE 2 ENRICHMENT COMPONENT
// Shows after Stage 1 completes. Collects follow-up questions
// then triggers Stage 3 full report generation.
// ═══════════════════════════════════════════════════════════

function Stage2Enrichment({ runId }: { runId: number }) {
  const [questions, setQuestions] = useState<Stage2Question[]>([]);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [stage3Started, setStage3Started] = useState(false);
  const { toast } = useToast();

  // Fetch Stage 2 questions on mount
  useEffect(() => {
    async function fetchQuestions() {
      try {
        const res = await apiRequest("GET", `/api/runs/${runId}/stage2-questions`);
        const data = await res.json();
        setQuestions(data.questions || []);
        // Initialize answers with empty strings
        const initial: Record<string, string> = {};
        (data.questions || []).forEach((q: Stage2Question) => {
          initial[q.id] = "";
        });
        setAnswers(initial);
      } catch (err) {
        console.error("Failed to fetch Stage 2 questions:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmitStage2 = async () => {
    ...
    try {
      ...
      const stage2Data = {
        pathwayResonance: answers["s2-1"] || "",
        capitalReadiness: answers["s2-2"] || "",
        decisionAuthority: answers["s2-3"] || "",
        summitParticipation: answers["s2-4"] || "",
        keyConcern: answers["s2-5"] || "",
        additionalAnswers: questions
          .filter(q => !["s2-1","s2-2","s2-3","s2-4","s2-5"].includes(q.id) && answers[q.id])
          .map(q => ({ questionId: q.id, question: q.question, answer: answers[q.id] })),
      };

      await apiRequest("POST", `/api/runs/${runId}/stage2`, stage2Data);

      // Trigger Stage 3 generation
      await apiRequest("POST", `/api/runs/${runId}/stage3`);

      setStage3Started(true);

      // Invalidate cache to start polling the new Stage 3 run
      queryClient.invalidateQueries({ queryKey: ["/api/runs", String(runId)] });

      toast({ title: "Full report generation started", description: "Stage 3 analysis is running with your enrichment data." });
    } catch (err: any) {
  ...
  const handleSkipToStage3 = async () => {
    ...
    try {
      ...
      setStage3Started(true);
      queryClient.invalidateQueries({ queryKey: ["/api/runs", String(runId)] });
      toast({ title: "Full report generation started", description: "Generating without enrichment data." });
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally {
      setSubmitting(false);
  ...
  if (stage3Started) {
    return (
      <div className="mt-8 text-center pb-8">
        ...
        <h3 className="text-lg font-semibold">Generating Full Report...</h3>
        <p className="text-sm text-muted-foreground mt-1">
          Stage 3 analysis is running. The page will update automatically.
        </p>
      </div>
    );
  }
    ...
    <div className="mt-10 mb-10">
      <Card className="border-2 border-primary/20 overflow-hidden">
        <div className="bg-primary/5 px-6 py-4 border-b border-primary/10">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Your initial analysis is ready above. Answer a few follow-up questions to generate a comprehensive full report with 15 detailed sections.
          </p>
        </div>
        <CardContent className="pt-6">
          <h3 className="font-semibold mb-4">Stage 2: Enrichment Questions</h3>
              ...
              {questions.map((q) => (
                <div key={q.id} className="space-y-2">
                  <label className="text-sm font-medium leading-tight block">
                    {q.question}
                    {q.mandatory && <span className="text-destructive ml-1">*</span>}
                  </label>
                  {q.context && (
              ...
              <div className="flex items-center gap-3 pt-4 border-t">
                ...
                >
                  {submitting ? (
                    ...
                    <>Generate Full Report (15 Sections)<ChevronRight className="h-4 w-4 ml-1" /></>
                  )}
                </Button>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleSkipToStage3}
                  disabled={submitting}
...
function StatusDot({ status }: { status: string }) {
  ...
  return (
    <span
      className={`w-1.5 h-1.5 rounded-full shrink-0 ${colors[status] || colors.queued}`}
    />
  );
}
...
function PhaseIndicator({
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      />
      <span className={active || done ? "text-foreground" : ""}>
        {label}
      </span>
    </div>
  );
